  ...
  switch (dispatchType) {
    case types.SHOW:
      if (currentState[modalName]?.isVisible) {
        // console.log(
        //   `${modalName} visibility is already ${currentState[modalName]?.isVisible}, so the dispatcher will not show.`,
        //   currentState
        // );
        return false;
      } else {
    ...
    case types.REMOVE:
      if (!currentState[modalName]?.isVisible) {
        // console.log(
        //   `${modalName} visibility is already ${currentState[modalName]?.isVisible}, so the dispatcher will not remove.`,
        //   currentState
        // );
        return false;
      } else {
    ...
    default:
      throw new Error(`Unknown dispatch type: ${dispatchType}`);
  }
};
...
export const useModals = ({ modals, props = {} }) => {
  ...
  const dismissedHandler = (obj) => {
    ...
    if (namedModals.map((m) => m.name).includes(obj.name)) {
      dispatch({
        type: types.REMOVE,
        name: obj.name,
      });
